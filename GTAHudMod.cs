        ...
        private Sprite BuildFistSprite()
        {
            ...
            for (int y = 0; y < size; y++)
            {
                for (int x = 0; x < size; x++)
                {
                    ...
                    // Wrist / cuff
                    if (x >= 10 && x <= 21 && y <= 6) filled = true;

                    // Main knuckle mass, rounded top corners
                    if (!filled && x >= 5 && x <= 26 && y >= 7 && y <= 27)
                    {
                        filled = true;
                        const int cornerR = 5;
                        if (y >= 27 - cornerR)
                    ...
                    // Knuckle notches along the top edge (separates 4 knuckles)
                    if (filled && y >= 23)
                    {
                        int[] notchCenters = { 11, 16, 21 };
                        foreach (int nc in notchCenters)
                        {
                            int dx2 = x - nc;
                            int dy2 = y - 28;
                            if (dx2 * dx2 + dy2 * dy2 <= 5) { filled = false; break; }
                        }
                    }

                    // Thumb, on the left side
                    if (!filled && x >= 0 && x <= 6 && y >= 10 && y <= 17) filled = true;
