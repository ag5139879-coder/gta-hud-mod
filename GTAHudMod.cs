...
namespace GTAHudMod
{
    public class Main : MelonMod
    {
        ...
        // HEALTH LOOKUP
        // ----------------------------------------------------------------
        private bool loggedHealthOutcome = false;
        ...
        private float GetHealthFraction()
        {
            try
            {
                ...
                if (cachedHealthComponent == null || healthRefreshTimer <= 0f)
                {
                    ...
                    if (!loggedHealthOutcome)
                    {
                        loggedHealthOutcome = true;
                        if (cachedHealthComponent == null)
                            MelonLogger.Warning("[HealthCheck] Player_Health component NOT FOUND in scene. Health bar will stay full.");
                        else
                            MelonLogger.Msg("[HealthCheck] Player_Health component found: " + cachedHealthComponent.GetType().FullName);
                    }
                }
            ...
            catch (Exception e)
            {
                ...
                {
                    loggedHealthOutcome = true;
                    MelonLogger.Warning("[HealthCheck] Exception reading health: " + e);
                }
            }
